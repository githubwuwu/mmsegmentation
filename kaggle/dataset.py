...
df_train = pd.read_csv(r"C:\Users\wuwul\Desktop\projects\kaggle\kaggle_UW-Madison\data\uw-madison-gi-tract-image-segmentation\train.csv")
...
all_image_files = sorted(glob.glob("C:\\Users\\wuwul\\Desktop\\projects\\kaggle\\kaggle_UW-Madison\\data\\uw-madison-gi-tract-image-segmentation\\train\\*\\*\\scans\\*.png"), key=lambda x: x.split("\\")[-3] + "_" + x.split("\\")[-1])
...
for day, group in tqdm(df_train.groupby("days")):
    ...
    for i in range(msks.shape[0]):
        ...
        cv2.imwrite(f"C:\\Users\\wuwul\\Desktop\\projects\\kaggle\\kaggle_UW-Madison\\data\\mmseg_train\\images\\{new_file_name}", img)
        cv2.imwrite(f"C:\\Users\\wuwul\\Desktop\\projects\\kaggle\\kaggle_UW-Madison\\data\\mmseg_train\\labels\\{new_file_name}", msk)

all_image_files = glob.glob("C:\\Users\\wuwul\\Desktop\\projects\\kaggle\\kaggle_UW-Madison\\data\\mmseg_train\\images\\*")
...
for fold, (train_idx, valid_idx) in enumerate(split):
    with open(f"C:\\Users\\wuwul\\Desktop\\projects\\kaggle\\kaggle_UW-Madison\\data\\mmseg_train\\splits\\fold_{fold}.txt", "w") as f:
        for idx in train_idx:
            f.write(os.path.basename(all_image_files[idx])[:-4] + "\n")
    with open(f"C:\\Users\\wuwul\\Desktop\\projects\\kaggle\\kaggle_UW-Madison\\data\\mmseg_train\\splits\\holdout_{fold}.txt", "w") as f:
